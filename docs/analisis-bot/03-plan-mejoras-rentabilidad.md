...
| T20 | Filtro de régimen **continuo** (ER en cada ciclo de WF2) | 3 | 🟡 paso 1 (OBSERVE) 2026-09-06 | El ER solo se evaluaba al lanzar. Paso 1: `/refresh` reevalúa el ER de cada grid en cada ciclo de WF2; si la tendencia persiste (ER > 0.35, `REGIME_FILTER_STRIKES_TO_ALERT=2` ciclos) loguea `TREND_REGIME` en `bot_health_events` y lo expone en la respuesta (`grid.regime`) — NO toca el grid. Constantes: `REGIME_FILTER_MODE="OBSERVE"` (+ umbral/listones) en `config_auto_params.py`; helper puro `calculate_efficiency_ratio()` en `indicators.py`; columnas auto-migradas `er_last` / `er_trend_strikes`. **Paso 2 (pendiente):** decidir qué accionar — RECENTER/CLOSE automático (reusar política OUT_OF_RANGE) o solo notificar — tras observar los avisos reales; también qué hacer si nadie responde al aviso. Ver §8 y `decisiones-tecnicas.md` T20. |
...
- **✅ Paso 1 (2026-09-03)** en rama `feat/t13-puerta-determinista-20260903`:
  `/auto-params` ahora devuelve `veto_reasons: []`. Espejo de los 4 criterios
  del LLM: (1) `ER > 0.35` (mercado en tendencia, de `ER_MAX_TRADEABLE`), (2)
  `leverage > 5x` con `ATR% > 2%`, (3) `top_3` vacío y (4)
